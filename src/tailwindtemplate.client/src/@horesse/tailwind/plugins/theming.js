...
// -----------------------------------------------------------------------------------------------------
// @ horesse TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
  ...
  (options) =>
    ({ addComponents, e, theme }) => {
            ...
            _.fromPairs(
              _.flatten(
                _.map(
                  ...
                  (value, key) => [
                    [`--horesse-${e(key)}`, value],
                    [`--horesse-${e(key)}-rgb`, chroma(value).rgb().join(',')]
                  ]
                )
              )
            )
      ...
      const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) => {
        ...
        const background = theme(`horesse.customProps.background.${colorScheme}`);
        const foreground = theme(`horesse.customProps.foreground.${colorScheme}`);
          ...
          colors: _.fromPairs(
            _.flatten(
              _.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                [name, `rgba(var(--horesse-${name}-rgb), <alpha-value>)`],
                [`on-${name}`, `rgba(var(--horesse-on-${name}-rgb), <alpha-value>)`]
              ])
            )
          )
        },
