...
import { HighlightService } from '@horesse/components/highlight/highlight.service';

@Component({
  selector       : 'textarea[horesse-highlight]',
  templateUrl    : './highlight.component.html',
  styleUrls      : ['./highlight.component.scss'],
  encapsulation  : ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
  exportAs       : 'horesseHighlight',
  standalone     : true,
  imports        : [NgClass]
})
export class HighlightComponent implements OnChanges, AfterViewInit {
  ...
  private _highlightAndInsert(): void {
    ...
    this.highlightedCode = this._domSanitizer.sanitize(
      ...
      this._highlightService.highlight(this.code, this.lang)
    );
